...
class HTMLRewriter {
    ...
    process(source, config = {}) {
        ...
        iterate(ast, node => {
            ...
            node.attrs.forEach(attr => {
                ...
                switch(handler) {
                    case 'url':
                        node.setAttribute(`ht-${attr.name}`, attr.value);
                    ...
                    case 'html':
                        ...
                        break;
                    case 'htParse':
                        let i = Math.round(Math.random() * 100000000)
                            switch(attr.value) {
                                case "_blank":
                                    node.setAttribute(`ht-${attr.name}`, `${attr.value}`)
                                    attr.value = "_self"
                                    let f = false
                                    for(var key in node.attrs) { if(node.attrs[key].name == "class") f = key }
                                    if(f != false) {
                                        node.setAttribute("class", node.attrs[f][1] + " a" + i)
                                    } else { 
                                        node.setAttribute("class", "a" + i)
                                    }
                                    node.setAttribute("href" + i, node.getAttribute("href"))
                                    node.setAttribute("href", "javascript:")
                                    node.setAttribute("onclick", "window.parent.newTab(document.getElementsByClassName(\"a" + i + "\")[0].getAttribute(\"href" + i + "\"))")
                                break
                                case "_parent" || "_top":
                                    node.setAttribute(`ht-${attr.name}`, `${attr.value}`)
                                    attr.value = "_self"
    ...
    source(processed, config = {}) {
        ...
        iterate(ast, node => {
            if (!node.tagName) return;
            node.attrs.forEach(attr => {
                if (node.hasAttribute(`ht-${attr.name}`)) { 
                    attr.value = node.getAttribute(`ht-${attr.name}`);
                    node.removeAttribute(`ht-${attr.name}`)
                };
            });
        });
        return parse5.serialize(ast);
        ...
        return [
            { 
                ...
                attrs: [
                    {
                        name: '#text',
                        value: `window.open=((n,o,e)=>{n||(n="https://presearch.org"),window.parent.newTab(n),console.log("window.open params:\n"+n+"\n"+o+"\n"+e)});`
                    },
                ],
            },
            { 
            ...
            { 
                ...
                childNodes: [
                    {
                        nodeName: '#text',
                        value: `window.$ht = new Corrosion({ window, codec: '${this.ctx.config.codec || 'plain'}',  prefix: '${this.ctx.prefix}', ws: ${this.ctx.config.ws}, cookie: ${this.ctx.config.cookie}, }); $ht.init(); document.currentScript.remove();`
                    },
                ], 
                attrs: [],
            }
