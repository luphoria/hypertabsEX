    ...
    return function rewriteDocument() {
            ...
            ctx.window.Element.prototype.setAttribute = new Proxy(ctx.window.Element.prototype.setAttribute, {
                apply: (target, that, args) => {
                    if (args[0] && args[1]) {
                        ...
                        switch(handler) {
                            case 'url':
                                Reflect.apply(target, that, [`ht-${args[0]}`, args[1]]);
                            ...
                            case 'htParse':
                                ...
                                let i = Math.round(Math.random() * 100000000)
                                    switch(args[1]) {
                                        case "_blank":
                                            args[1] = "_self"
                                            let f = false
                                            for(var key in that.attributes) { if(that.attributes[key].name == "class") f = key }
                                            if(f != false) {
                                                that.setAttribute("class", that.attributes[f][1] + " a" + i)
                                            } else { 
                                                that.setAttribute("class", "a" + i)
                                            }
                                            that.setAttribute("href" + i, that.getAttribute("href"))
                                            that.setAttribute("href", "javascript:window.parent.newTab(document.getElementsByClassName(\"a" + i + "\")[0].getAttribute(\"href" + i + "\"))")
                                            // that.setAttribute("onclick", "window.open(document.getElementsByClassName(\"a" + i + "\")[0].getAttribute(\"href" + i + "\"))")
                                        break
                                        case "_parent" || "_top":
                                            args[1] = "_self"
                                            break
                                ...
                                break;
                            case 'delete':
                                return Reflect.apply(target, that, [`ht-${args[0]}`, args[1]]);
                        };
                    };
                    return Reflect.apply(target, that, args);
                },
        ...
        if (ctx.window.Element.prototype.getAttribute) {
            ctx.window.Element.prototype.getAttribute = new Proxy(ctx.window.Element.prototype.getAttribute, {
                apply: (target, that, args) => {
                    if (args[0] && that.hasAttribute(`ht-${args[0]}`)) args[0] = `ht-${args[0]}`;
                    return Reflect.apply(target, that, args);
                },
            });
        }; 
                            ...
                            apply: (target, that, args) => {
                                ...
                                switch(entry.handler) {
                                    ...
                                    case 'srcset':
                                        ...
                                        break;
                                    case 'delete':
                                        val = that.getAttribute(`ht-${property}`);
                                        break;
                                    case 'window':
                                        try {
                                            if (!val.$ht)  {
                                                val.$ht = new ctx.constructor({ ...ctx.config, window: val, });
                                                val.$ht.init();
                                                val.$ht.meta = ctx.meta;
                                            };
                                        } catch(e) {};
                                };
                                return val;
